...
Usage:
    python scripts/search_topic.py \
        ...
        --rubric-file data/interim/quantization_rubric.txt \
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Hybrid search + LLM classification for one topic.")
    parser.add_argument("--db", type=Path, default=DB_PATH)
    parser.add_argument("--topic", required=True, help="Topic name, e.g. 'quantization' — key in topic_subset.")
    parser.add_argument("--query", required=True, help="Natural-language query for the hybrid search.")
    parser.add_argument("--categories", nargs="+", required=True)
    parser.add_argument("--rubric-file", type=Path, required=True)
    parser.add_argument("--golden-set", type=Path, required=True, help="Labeled parquet for few-shot examples.")
    parser.add_argument("--top-k", type=int, default=500, help="Hybrid-ranked candidates sent to the LLM classifier.")
    parser.add_argument("--n-few-shot", type=int, default=3, help="Positive/negative examples each, from the golden set.")
    ...
    try:
        ...
        if len(corpus) == 0:
            log.error("No embedded papers found — run embed_abstracts.py for these categories first.")
            return
        ...
        top_idx = np.argsort(-fused)[: args.top_k]
        ...
        few_shot_df = select_few_shot_examples(golden, n_positive=args.n_few_shot, n_negative=args.n_few_shot)
        few_shot_examples = [
