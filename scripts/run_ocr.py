...
from scirex.ocr.modern.pipeline import (
    pdf_to_images, ocr_images, make_paper_dir, save_all, update_db,
)
...
def main():
    ...
    parser.add_argument("--input-dir", type=Path, default=Path("data/raw/pdfs"), help="Folder containing PDFs.")
    parser.add_argument("--db", type=Path, default=Path("data/arxiv_metadata.duckdb"), help="DuckDB database file.")
    parser.add_argument("--keyword", type=str, required=True, help="Topic/subset of this batch (stored in paper_local.keyword_for_ocr).")
    parser.add_argument("--limit", type=int, default=None, help="Optional: process only the first N PDFs (for testing).")
    parser.add_argument("--max-concurrent-pdfs", type=int, default=6, help="Number of PDFs to OCR in parallel (default 6).")
